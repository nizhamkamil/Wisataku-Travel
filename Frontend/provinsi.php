...
<?php 
    ...
    $kodekonten = $_GET['provinsi']; 
    $detilprovinsi = mysqli_query($con,"SELECT * FROM provinsi,hotel WHERE provinsi.provinsiID = hotel.provinsiID AND provinsi.provinsiID ='$kodekonten' LIMIT $mulaitampilhotel, $jumlahtampilhotel" );
    $detilprovinsi1 = mysqli_query($con,"SELECT provinsiNama FROM provinsi WHERE provinsiID ='$kodekonten'" );
?>
...
<body>
    ...
    <!-- GALLERY HOTEL START -->
    <div class="container">
        <?php if (mysqli_num_rows($detilprovinsi) > 0 ){ ?>
        <?php $rowprovinsi1 = mysqli_fetch_array($detilprovinsi1) ?>
        <h1 style="text-align: center; margin-top:30px">Hotel di Provinsi : <?php echo $rowprovinsi1['provinsiNama'] ?>
        </h1>
        <div class="row">
            <?php while($rowfotohotel = mysqli_fetch_array($detilprovinsi)){ ?>
            <div class="col-md-6">
            ...
            <h1>Maaf belum ada hotel terdaftar di provinsi ini</h1>
            <?php }?>
        </div>
    </div>
    <!-- GALLERY HOTEL END -->
